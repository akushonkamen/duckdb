#!/bin/bash
# Build script for AI Extension with COVERAGE
# This script compiles the AI extension with coverage flags for gcov analysis
...
# Compilation flags for loadable extension with COVERAGE
...
# COVERAGE: -O0 for debug, -g for symbols, --coverage for gcov data
CXX="clang++"
CXXFLAGS="-std=c++17 -fPIC -O0 -g --coverage -DDUCKDB_EXTENSION_PLATFORM_NAME=${PLATFORM} -DDUCKDB_BUILD_LOADABLE_EXTENSION"
LDFLAGS="--coverage"
...
echo "Compiling AI Extension (loadable mode with COVERAGE)..."
${CXX} ${CXXFLAGS} ${INCLUDES} -shared -undefined dynamic_lookup ${SOURCES} ${LDFLAGS} -o ${OUTPUT}
